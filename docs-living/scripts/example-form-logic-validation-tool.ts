...
console.log(`
FormLogicValidation Tool Example Script
=======================================
...
REQUIRED ENVIRONMENT SETUP:
  ...
  2. Run: npx ts-node docs-living/scripts/example-form-logic-validation-tool.ts
...
/**
 * Example: Using the FormLogicValidation tool via marvToolSet executor
 * This demonstrates the actual tool execution using the tool definitions and executor
 */
async function exampleFormLogicValidationTool() {
  console.log(`
FormLogicValidation Tool Example
--------------------------------

Using marvToolSet.executeToolCall to run the FormLogicValidation tool
Tool: ${FsRestrictedApiRoutesEnum.FormLogicValidation}
...
`);
  ...
  try {
    ...
    const toolDefinition = marvToolSet.toolDefinitions.find(
      (tool) => tool.name === FsRestrictedApiRoutesEnum.FormLogicValidation,
    );
...
async function main() {
  console.log(`
Starting FormLogicValidation tool example...
Using test form ID: ${FORM_ID}
`);

  try {
    await exampleFormLogicValidationTool();
...
export { exampleFormLogicValidationTool };
