...
function removeEmbeddings(obj: any): any {
  if (Array.isArray(obj)) {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (key.includes('embedding') || key === 'vector_graph' || key === 'chunk_embedding') {
        continue;
      }
...
async function main() {
  try {
    ...
    const preQueryResponse = await makeRequest(`${API_BASE}/preQuery`, preQueryData);
    ...
    switch (SEARCH_TYPE) {
      ...
      case 'free-text-search':
        ...
        searchData.freeText = preQueryResponse.freeText || preQueryResponse.keywords || ['text', 'search'];
        console.log(`🔍 Free text search data: freeText=${JSON.stringify(searchData.freeText)}`);
        break;
      default:
        throw new Error(`Unknown search type: ${SEARCH_TYPE}`);
    }
    
    const searchResponse = await makeRequest(`${API_BASE}/${SEARCH_TYPE}`, searchData);
    ...
    const contextDocsCount = searchResponse['CONTEXT-DOCUMENTS'] ? searchResponse['CONTEXT-DOCUMENTS'].length : 0;
    ...
    if (totalCount > 0) {
      ...
      console.log(`📊 Distribution: SLACK ${slackPercent}%, CONTEXT-DOCUMENTS ${contextPercent}%`);
      
      // Show first result from each knowledge base if available
      if (slackCount > 0) {
        const firstSlack = searchResponse.SLACK[0];
        console.log(`💬 First SLACK result confidence: ${firstSlack.confidence || 'N/A'}`);
      }
      if (contextDocsCount > 0) {
        const firstContext = searchResponse['CONTEXT-DOCUMENTS'][0];
        console.log(`📄 First CONTEXT-DOCUMENTS result confidence: ${firstContext.confidence || 'N/A'}`);
      }
    } else {
      console.log('⚠️  No results found - check channel configuration and data availability');
    }
