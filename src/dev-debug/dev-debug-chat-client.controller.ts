...
export class DevDebugChatClientController {
  ...
  async createSession(@Res() res: Response): Promise<void> {
    try {
      ...
      const sessionResult =
        this.authPermissionsService.createDevUserAndSession();
   ...
   */
  @Get('monitor')
  async serveMonitorPage(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<void> {
    try {
      // Check for existing session, create one if it doesn't exist
      let session;
      const jwtToken = req.cookies['dev-jwt'];

      if (jwtToken) {
        session = this.authPermissionsService.getDevSessionByJwtToken(jwtToken);
      }

      if (!session) {
        // Auto-create a dev session
        const sessionResult =
          this.authPermissionsService.createDevUserAndSession();
  ...
  async serveChatPage(
    ...
    try {
      ...
      const session =
        this.authPermissionsService.getDevSessionByJwtToken(jwtToken);
      if (!session) {
        res.redirect('/dev-debug/create-session');
        return;
      }
    ...
    try {
      ...
      return {
        ...
        conversations: conversations.map((conv) => ({
          ...
          currentRobot: 'none', // TODO: Add currentRobot to Conversation interface
...
<body>
    <div class="header">
        <div>
            ...
            <p>Current Robot: <strong id="current-robot">none</strong> 
