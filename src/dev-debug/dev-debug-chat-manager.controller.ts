...
export class DevDebugChatManagerController {
  ...
  async startConversation(@Req() req: any): Promise<any> {
    ...
    try {
      ...
      const description =
        req.body?.description || 'Dev/Debug test conversation';
  ...
  async testConversationMessage(@Req() req: any): Promise<any> {
    ...
    try {
      ...
      const intentResult =
        await this.intentParsingService.parsePromptIntent(testMessage);
      ...
      const mockSlackCallback = async (content: {
        type: 'text';
        payload: string;
      }) => {
        this.logger.log('=== MOCK SLACK RESPONSE ===');
        this.logger.log(
          `Robot response: ${content.payload.substring(0, 100)}...`,
        );
      ...
      return {
        ...
        conversationDetails: {
          currentRobot: 'unknown', // TODO: Add currentRobot to Conversation interface
        },
      };
    } catch (error) {
  ...
  async testSlackFlow(@Req() req: any): Promise<any> {
    ...
    try {
      ...
      const testMessage =
        req.body?.message ||
        'Hello, I need help with form 12345. Can you check for errors?';
      this.logger.log(`Test message: ${testMessage}`);
