...
describe('RobotChatAnthropicTools', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('RobotChatAnthropicToolSet Structure', () => {
    it('should have correct structure', () => {
      ...
      expect(Array.isArray(RobotChatAnthropicToolSet.toolDefinitions)).toBe(
        true,
      );
    ...
    it('should have valid tool definitions structure', () => {
      RobotChatAnthropicToolSet.toolDefinitions.forEach((tool) => {
  ...
  describe('Individual Tool Definitions', () => {
    describe('sumo_logic_query', () => {
      const sumoLogicTool = RobotChatAnthropicToolSet.toolDefinitions.find(
    ...
    describe('sso_autofill_assistance', () => {
      const ssoTool = RobotChatAnthropicToolSet.toolDefinitions.find(
    ...
    describe('form_and_related_entity_overview', () => {
      const formOverviewTool = RobotChatAnthropicToolSet.toolDefinitions.find(
  ...
  describe('handleFormAndRelatedEntityOverview', () => {
    it('should set API key when provided', async () => {
      ...
      const result = await RobotChatAnthropicToolSet.executeToolCall(
        'form_and_related_entity_overview',
        {
          formId: '12345',
          apiKey: 'test-api-key',
      ...
      expect(mockFsApiClient.setApiKey).toHaveBeenCalledWith('test-api-key');
    ...
    it('should not set API key when not provided', async () => {
      ...
      await RobotChatAnthropicToolSet.executeToolCall(
        'form_and_related_entity_overview',
        {
          formId: '12345',
        },
      ...
      expect(mockFsApiClient.setApiKey).not.toHaveBeenCalled();
    });
