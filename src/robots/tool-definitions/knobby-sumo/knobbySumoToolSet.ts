...
// Configuration
const INFO_SERVICES_BASE_URL = process.env.INFORMATION_SERVICES_URL || 'http://localhost:3505/information-services';

const getAuthToken = () => {
  return process.env.INFORMATION_SERVICES_TOKEN || 'istack-buddy-dev-token-2024';
};
...
const makeInfoServicesRequest = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Information Services API error: ${response.status} ${response.statusText} - ${errorText}`);
    }

    return await response.json();
  } catch (error) {
    console.error(`KnobbySumo API Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  try {
    ...
    switch (toolName as KnobbySumoToolsEnum) {
      case KnobbySumoToolsEnum.SubmitQuery:
        {
          result = await makeInfoServicesRequest('/context-sumo-report/query/submit', 'POST', toolArgs);
        }
        break;
      ...
      case KnobbySumoToolsEnum.GetJobStatus:
        {
          ...
          result = await makeInfoServicesRequest(`/context-sumo-report/query/${jobId}/status`, 'GET');
        }
        break;
      ...
      case KnobbySumoToolsEnum.GetJobResults:
        {
          const { jobId } = toolArgs;
          result = await makeInfoServicesRequest(`/context-sumo-report/query/${jobId}/results`, 'GET');
        }
        break;
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          queryName: {
            type: 'string',
            enum: ['submitActionReport', 'submissionCreatedForForm', 'authProviderMetrics'],
          ...
          subject: {
            ...
            properties: {
              ...
              submitActionType: { type: 'string', description: 'Submit action type filter' },
              startDate: { type: 'string', description: 'Start date (YYYY-MM-DD)' },
              endDate: { type: 'string', description: 'End date (YYYY-MM-DD)' },
              accountId: { type: 'string', description: 'Account ID filter' },
              submissionId: { type: 'string', description: 'Submission ID filter' }
            }
          },
          isValidationOnly: {
            type: 'boolean',
