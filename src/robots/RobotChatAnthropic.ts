...
`;
   ...
   */
  private executeToolCall(toolName: string, toolArgs: any): string {
    return RobotChatAnthropicToolSet.executeToolCall(toolName, toolArgs);
    // switch (toolName) {
    //   case 'sumo_logic_query':
    //     return this.handleSumoLogicQuery(toolArgs);
    //   case 'sso_autofill_assistance':
    //     return this.handleSsoAutofillAssistance(toolArgs);
    //   default:
    //     return `❌ Unknown tool: ${toolName}. Available tools: sumo_logic_query, sso_autofill_assistance`;
    // }
  }
    ...
    try {
      ...
      for (const toolUse of toolUseBlocks) {
        try {
          ...
          const toolResult = this.executeToolCall(toolUse.name, toolArgs);
          chunkCallback(`\n\n${toolResult}`);
        } catch (error) {
          chunkCallback(
  ...
  public async acceptMessageImmediateResponse(
    ...
    try {
      ...
      for (const toolUse of toolUses) {
        try {
          const toolResult = this.executeToolCall(toolUse.name, toolUse.input);
          responseText += `\n\n${toolResult}`;
        } catch (error) {
          responseText += `\n\nError executing tool ${toolUse.name}: ${error instanceof Error ? error.message : 'Unknown error'}`;
        }
