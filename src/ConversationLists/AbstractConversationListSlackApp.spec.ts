...
class TestConversationList extends AbstractConversationListSlackApp<TConversationListItem> {
...
describe('AbstractConversationList', () => {
  ...
  describe('canInteractWithRobots static method', () => {
    it('should identify roles that can interact with robots', () => {
      expect(
        AbstractConversationListSlackApp.canInteractWithRobots('cx-agent'),
      ).toBe(true);
      expect(
        AbstractConversationListSlackApp.canInteractWithRobots('cx-supervisor'),
      ).toBe(true);
      expect(
        AbstractConversationListSlackApp.canInteractWithRobots(
          'conversation-admin',
        ),
      ).toBe(true);
      expect(
        AbstractConversationListSlackApp.canInteractWithRobots('cx-customer'),
      ).toBe(false);
      expect(
        AbstractConversationListSlackApp.canInteractWithRobots('robot'),
      ).toBe(false);
    });
  });
});
